...
function sleepCountdown(seconds,pagesDone,pages){return new Promise(resolve=>{const deadline=Date.now()+Math.ceil(seconds)*1000;let timerId=null;let done=false;const finish=val=>{if(done)return;done=true;clearTimeout(timerId);document.removeEventListener('visibilitychange',onVisible);resolve(val)};const tick=()=>{if(done)return;if(stopRequested){$('nextPageTimer').textContent='Parado';$('totalTimer').textContent='Parado';return finish(false)}const remaining=Math.max(0,Math.ceil((deadline-Date.now())/1000));$('nextPageTimer').textContent=formatTime(remaining);$('totalTimer').textContent=formatTime(remaining+estimatedWaitAfter(pagesDone+1,pages));if(remaining<=0)return finish(true);timerId=setTimeout(tick,document.visibilityState==='hidden'?Math.max(1000,deadline-Date.now()):1000)};const onVisible=()=>{if(document.visibilityState==='visible'){clearTimeout(timerId);tick()}};document.addEventListener('visibilitychange',onVisible);tick()})}
...
$('generate').addEventListener('click',async()=>{
  ...
  if(!Number.isInteger(pages)||pages<1||pages>MAX_PAGES){show('error');$('error').textContent='Informe uma quantidade inteira de páginas entre 1 e 40.';return}
  const btn=$('generate');
  $('pages').disabled=true;
  stopRequested=false;
  $('stop').disabled=false;
  ...
  try{
    if(typeof navigator!=='undefined'&&'locks' in navigator){await new Promise(resolve=>{navigator.locks.request('cryptos-collection',()=>new Promise(r=>{releaseCollectionLock=r;resolve()}));});}
    ...
    $('stop').disabled=false;
    if(releaseCollectionLock){releaseCollectionLock();releaseCollectionLock=null;}
  }
});
